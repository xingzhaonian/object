...
class Assemble(object):
    ...
    def PutBytes(self, ints):
        self.bytes_data += ints.to_bytes(1, byteorder='little', signed = True)[::-1]
    
    # Bool类型 --1个字节
    def PutBool(self, bool):
        if bool:
            self.bool_bytes = 1
            self.bytes_data += self.bool_bytes.to_bytes(1, byteorder='little', signed = True)[::-1]
        else:
            self.bool_bytes = 0
            self.bytes_data += self.bool_bytes.to_bytes(1, byteorder='little', signed = True)[::-1]
    ...
    def PutShort(self, shortnum):
        self.shortint_bytes = shortnum.to_bytes(2, byteorder='little', signed = True)[::-1]
        self.bytes_data += self.shortint_bytes

    # int类型 --4个字节
    def PutInt(self, num):
        self.num_bytes = num.to_bytes(4, byteorder='little', signed = True)[::-1]
        self.bytes_data += self.num_bytes
    
    # int类型 --8个字节
    def PutLong(self, longnum):
        self.longnum_bytes = longnum.to_bytes(8, byteorder='little', signed = True)[::-1]
    ...
    #def CG_verifyResult(self):
        ...
        # 加密
    #    self.encryption = 0
    #    self.encryption_bytes = self.encryption.to_bytes(1, byteorder='little', signed = True)[::-1]

        # 消息长度
    #    self.bytes_data_length =len(self.bytes_data)
    #    self.bytes_data_length_bytes = self.bytes_data_length.to_bytes(4, byteorder='little', signed = True)[::-1]
    ...
    def Result(self):
        ...
        # 消息长度
        self.message_length = len(self.bytes_data)
        self.message_length_bytes = self.message_length.to_bytes(4, byteorder='little', signed=True)[::-1]

        # 数据 = 加密 + 消息长度 + 消息体
        self.bytes_data = self.encryption_bytes + self.message_length_bytes + self.bytes_data
