...
LEGB规则:
L 是 local (局部作用域--->函数内部的变量)
E 是 enclosed (嵌套函数的外层函数作用域--->闭包(外层函数的变量))
G 是 global (全局作用域--->该脚本中的变量)
B 是 build-in (python内置函数, 内置作用域--->python内置函数的名称, 如果该名称被覆盖就无法使用该函数了)
...
高阶函数
一个函数接收另一个函数作为参数时, 或者一个函数的返回值为另一个函数(若返回值为该函数本身，则为递归), 满足其中一种则为高阶函数
例如: 
def functionA():
	print('调用函数A')
...
===================================================================================================
生成器
函数一般在return完成后就结束了, 函数所做的工作以及保存在局部变量中的数据也将丢失, 再次调用的时候, 一切都是从头再来,
但是使用闭包可以保存外层函数的变量, 但是闭包的定义又稍微复杂一点, 这时候yield语句就出现了
定义一个生成器很简单, 使用yield 语句来代替函数中的 return语句, 可以把生成器看作一个制作机器
def fib():
	a, b = 0, 1
    while True:
		a, b = b, a + b
...
f = fib()   此时 f 就是一个生成器 <generator object fib at 0x000001E3D31741E0>
可以调用next(f)来取出f的值, 也可以用for循环来取出, 每次取出的时候, 都会保留函数内部的状态

生成器表达式
将列表推导式的方括号改为小括号就变成了列表推导式
s = (i ** 2 for i in range(10))
此时s就是一个生成器, 生成器表达式和列表推导式最大的不同就是 列表推导式会一下子将所有数据生产出来, 并放到一个列表中
而生成器表达式就像一只母鸡, 一次只下一只蛋
=====================================================================================================
递归
递归就是函数调用自身的过程
要让递归正常工作, 必须要有一个结束条件, 并且每次调用都会向着这个条件的结束去推进
递归事实上是很耗费资源的
def get_sum(n):
    if n == 1:
        return n
