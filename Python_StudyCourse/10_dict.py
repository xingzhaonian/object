...
字典也可以指定对key或者value进行排序, 比如: 
指定对key进行排序: dic = {'语文':80, '数学':88, '英语':87}; sorted(dic.items(), key = lambda s:s[0])
指定对value进行排序 dic = {'语文':80, '数学':88, '英语':87}; sorted(dic.items(), key = lambda s:s[1])
